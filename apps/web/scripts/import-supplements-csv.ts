import "dotenv/config";

import { readFile } from "node:fs/promises";
import { resolve } from "node:path";

import { prisma } from "@/lib/db/prisma";
import { parseSupplementCsv } from "@/lib/supplementation/supplement-csv-parser";
import {
  applySupplementCsvImport,
  createDryRunSupplementImportRepository,
  createPrismaSupplementImportRepository
} from "@/lib/supplementation/supplement-import-writer";

const args = parseArgs(process.argv.slice(2));
const inputPath = args.file ?? args._[0];

if (!inputPath) {
  throw new Error(
    "Supplement CSV file is required. Example: pnpm --dir apps/web supplement:import:csv ./supplements.csv"
  );
}

const rows = parseSupplementCsv(await readFile(resolve(inputPath), "utf8"));
const dryRun = !args.commit;
const result = await applySupplementCsvImport({
  rows,
  repository: getRepository(dryRun),
  dryRun
});

console.log("Supplement CSV import");
console.log(`mode: ${dryRun ? "dry-run" : "commit"}`);
console.log(`file: ${inputPath}`);
console.log(`rows: ${rows.length}`);
console.log(`creates: ${result.plan.create.length}`);
console.log(`updates: ${result.plan.update.length}`);
console.log(`skipped: ${result.plan.skipped.length}`);

if (result.createdIds.length || result.updatedIds.length) {
  console.log(`createdIds: ${result.createdIds.join(", ") || "none"}`);
  console.log(`updatedIds: ${result.updatedIds.join(", ") || "none"}`);
}

await prisma.$disconnect();

function getRepository(dryRunMode: boolean) {
  if (process.env.DATABASE_URL) {
    return createPrismaSupplementImportRepository(prisma);
  }

  if (dryRunMode) {
    return createDryRunSupplementImportRepository();
  }

  throw new Error("DATABASE_URL is required when running supplement imports with --commit.");
}

function parseArgs(argv: string[]) {
  const parsed: { _: string[]; commit?: boolean; file?: string } = { _: [] };

  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];
    if (arg === "--commit") {
      parsed.commit = true;
    } else if (arg === "--file") {
      parsed.file = argv[index + 1];
      index += 1;
    } else {
      parsed._.push(arg);
    }
  }

  return parsed;
}
