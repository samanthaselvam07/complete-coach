...
- `POST /api/v1/training-program-templates`: creates a template with validated JSON days/exercises. Body includes `name`, optional `description`, optional `goal`, `durationWeeks`, `status`, and `template.days[].exercises[]` entries with `exerciseId`, `exerciseName`, `sets`, `reps`, optional `restSeconds`, optional `rpe`, optional `rir`, optional `section` (`warmUp`, `workout`, `coolDown`), optional `tempo`, optional `cues`, and optional `notes`. `template.instructions` may store builder-level workout instructions.
