...
- `GET /api/v1/packages`: lists active-organization packages. Query: optional `status`, optional `limit`. Each package includes derived `activeSubscriptions`, `projectedMonthlyRevenue`, `customerLtv`, `ltvCustomerCount`, and periodized `customerMetrics` for monthly, quarterly, and annual views. Churn is calculated from archived clients (`lostCustomers / customersAtStart`) and Customer LTV uses ARPU x gross margin percentage / churn rate.
...
- `GET /api/v1/platform-billing/status`: returns the active organization's Complete Coach platform subscription state, access rule result, current Core/Scale plan, current period end, and usage against team-seat/client limits. Requires `payments:read`. Access rules: `active` and `trialing` allow access; `not_started`, `incomplete`, `incomplete_expired`, `past_due`, `unpaid`, `paused`, and `canceled` block access. Team-seat usage counts active owner/admin/coach/assistant memberships, so changing an existing member's role does not change seat usage.
- `POST /api/v1/platform-billing/checkout`: creates a Stripe Checkout subscription session on the Complete Coach Stripe account for `planId` (`core` or `scale`). Body: `planId`, optional `successUrl`, optional `cancelUrl`; safe relative paths are resolved server-side. Requires `payments:manage` and `STRIPE_SECRET_KEY`.
